...
''',
    ...
    parser.add_argument(
        'source',
        help='Source folder to organize (required)'
    )

    # Core options
    parser.add_argument(
        '--dest', '-d',
        help='Destination root folder (default: source folder)'
    )
    parser.add_argument(
        '--recursive', '-R',
        action='store_true',
    ...
    parser.add_argument(
        '--clean', '-C',
        action='store_true',
        help='Remove empty directories in source after organization'
    )

    # Rule options
    parser.add_argument(
        '--rules', '-r',
        help='Custom rules as a Python dictionary string, e.g. "{\'.txt\': \'Docs\'}"'
    )
    parser.add_argument(
        '--rules-file',
        help='JSON file containing custom rules'
    )
    parser.add_argument(
        '--combine', '-c',
        action='store_true',
    ...
    parser.add_argument(
        '--config',
        help='Path to a JSON configuration file (overrides defaults, but CLI arguments take precedence)'
    )

    # Logging options
    parser.add_argument(
        '--log-file',
        help='Path to log file (if not set, logs go only to console)'
    )
    parser.add_argument(
        '--stream-level', '-sl',
        default='debug',
    ...
    parser.add_argument(
        '--style',
        default='simple',
        help='Log style name (defined in styles file, default: simple)'
    )
...
def main() -> None:
    ...
    try:
        ...
        print("\nInterrupted by user.", file=stderr)
        exit(1)
    except Exception as e:
        # In case logging wasn't set up yet, we print to stderr
        print(f"Fatal error: {e}", file=stderr)
        exit(2)