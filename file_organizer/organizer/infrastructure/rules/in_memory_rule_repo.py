...
class InMemoryRuleRepository(RuleRepository):
    """
    Builds a RuleSet from a dictionary provided in memory.
    Useful for handling rules passed directly via CLI (--rules) or for testing.
    Can also combine with a default repository.
    """
    ...
    def __init__(
        ...
        """
        Args:
            rules_dict: Dictionary containing the rules configuration.
                Expected to have the same structure as the JSON file.
            default_repo: Repository to load default rules from (if combine=True).
            combine: If True, merge default rules with the provided rules_dict.
        """
    ...
    def load_rules(self) -> RuleSet:
        # Build user rules from the dict
        user_rules = self._build_rules_from_dict(self.rules_dict)

        if self.combine and self.default_repo:
            default_set = self.default_repo.load_rules()
            # Combine: user rules first, then default (user has higher priority)
            combined_rules = user_rules + default_set.rules
            # For other settings, user takes precedence
            other_behavior = self.rules_dict.get('other_behavior', default_set.other_behavior)
            ignore_extensions = self.rules_dict.get('ignore_extensions', default_set.ignore_extensions)
            ignore_size_more = self.rules_dict.get('ignore_size_more_than', default_set.ignore_size_more_than)
            ignore_size_less = self.rules_dict.get('ignore_size_less_than', default_set.ignore_size_less_than)
            merged_dict = {
            ...
            # Just use the user rules and settings
            config = self.rules_dict.copy()
