...
def test_in_memory_basic_loading(sample_rules_dict, parent_dir):
    ...
    assert rule_set.get_target_folder(txt_file) == 'Documents'
    assert rule_set.get_target_folder(jpg_file) == 'Images'
    assert rule_set.get_target_folder(unknown_file) == 'Other'
...
def test_in_memory_ignore_by_extension(sample_rules_dict, parent_dir):
    ...
    assert rule_set.get_target_folder(bak_file) is None
...
def test_in_memory_ignore_by_size(sample_rules_dict, parent_dir):
    ...
    assert rule_set.get_target_folder(small_file) is None
    ...
    assert rule_set.get_target_folder(huge_file) is None
...
def test_in_memory_priority(sample_rules_dict, parent_dir):
    ...
    assert rule_set.get_target_folder(script_file) == 'MediumFiles/Scripts'
...
def test_in_memory_composite_and(sample_rules_dict, parent_dir):
    ...
    assert rule_set.get_target_folder(good_script) == 'MediumFiles/Scripts'
    ...
    assert rule_set.get_target_folder(wrong_size_script) is None
    ...
    # Size rule matches
    assert rule_set.get_target_folder(data_file) == 'MediumFiles'
...
def test_in_memory_other_behavior_raise(sample_rules_dict, parent_dir):
    ...
    with pytest.raises(RuleNotFoundError):
        rule_set.get_target_folder(unknown_file)
...
def test_in_memory_other_behavior_ignore(sample_rules_dict, parent_dir):
    ...
    unknown_file = FileItem(Path('/tmp/unknown.xyz'), parent_dir)
    assert rule_set.get_target_folder(unknown_file) is None
...
def test_in_memory_combine_with_default(parent_dir):
    ...
    assert rule_set.get_target_folder(py_file) == 'PythonScripts'
    # .txt should still match
    assert rule_set.get_target_folder(txt_file) == 'Texts'
